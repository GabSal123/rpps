...
namespace RPPS.Models
{
    ...
    public class CargoObj
    {
        ...
        public static List<CargoObj> GetOrderCargos(int orderId) {
            var cargos = new List<CargoObj>();
        ...
        public static void FreeCargosFromOrder(int orderId) {
            using var connection = new MySqlConnection(_connectionString);
            connection.Open();

            using var command = new MySqlCommand("UPDATE cargoobj SET orderid = NULL WHERE orderid = @OrderId", connection);
            command.Parameters.AddWithValue("@OrderId", orderId);
